...
public class RED extends LinearOpMode {
    ...
    boolean autoIsRed = false;
    Hardware robot = Hardware.getInstance();
    int step = 0;
    // methods for opening/closing the claw and raising/lowering gecko arm
    ...
    public void runOpMode() {
        ...
        // runs script for Autonomous mode
        waitForStart();
        while (opModeIsActive()) {
            autoScript();
            telemetry.addData("Moving to step", step);
            telemetry.addData("Is auto red?", autoIsRed);
            telemetry.update();
        }
    }
